...
const getEnvInt = (key, def) => {
    const v = process.env[key];
    if (v == null) return def;
    const n = parseInt(v, 10);
    return Number.isFinite(n) ? n : def;
};
const getEnvStr = (key, def) => {
    const v = process.env[key];
    return (v == null || v === '') ? def : v;
};

// 重试机制的配置参数（可通过环境变量覆盖）
// AI_RETRY_MAX_RETRIES: 每个提供商的最大重试次数
// AI_RETRY_DELAY_MS: 每次重试之间的延迟
// AI_RETRY_MAX_TOTAL_ATTEMPTS: 在所有提供商之间的最大总尝试次数
const RETRY_CONFIG = {
    maxRetries: getEnvInt('AI_RETRY_MAX_RETRIES', 2),
    retryDelay: getEnvInt('AI_RETRY_DELAY_MS', 100),
    maxTotalAttempts: getEnvInt('AI_RETRY_MAX_TOTAL_ATTEMPTS', 8),
};
...
Object.keys(providerModelConfigs).forEach(provider => {
    ...
    if (providerModels) { // 确保providerModels存在
        Object.keys(providerModels).forEach(modelName => {
            const modelConfig = providerModels[modelName];
            if (modelConfig && modelConfig.aliases) {
                modelConfig.aliases.forEach(alias => {
...
const DEFAULT_TIMEOUT = getEnvInt('AI_DEFAULT_TIMEOUT_MS', 240000);
...
const modelTimeouts = {
    ...
    'gpt-oss-120b': 180000, // 3分钟
    'gpt-5-nano': 120000, // 2分钟
};
...
async function callProviderWithRetry(provider, messages, modelName, timeout, logger) {
    ...
    while (retryCount < RETRY_CONFIG.maxRetries) {
        try {
            ...
            // 对于 Yuanbao 供应商，传递原始模型名称以便正确识别图片模型
            let response;
            if (provider === 'Yuanbao') {
                response = await withTimeout(apiFunction(messages, providerModelName, modelName), timeout);
            } else {
                response = await withTimeout(apiFunction(messages, providerModelName), timeout);
            }
...
const retryWithFallback = async (messages, modelName, availableProviders, timeout, logger) => {
    ...
    if (availableProviders.length === 1) {
        ...
        return result.success ? result.response?.trim() : '指定服务商调用失败';
    }
...
class Logger {
    constructor(options = {}) {
        // 日志级别设置，支持通过环境变量 AI_LOG_LEVEL 配置
        this.logLevel = options.logLevel || getEnvStr('AI_LOG_LEVEL', 'debug'); // debug, info, warn, error
    }
