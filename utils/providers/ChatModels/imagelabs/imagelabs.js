...
class ImageLabs {
    ...
    async #fetch(url, data, headers, proxy) {
        const fetchOptions = {
            ...
            // 代理设置：
            // 1) 如果上层传入了 proxy 字符串（例如 http://127.0.0.1:7890），优先使用该代理。
            // 2) 否则自动从环境变量读取代理（AI_PROXY_URL / HTTPS_PROXY / HTTP_PROXY / ALL_PROXY）。
            ...(proxy
                ? { agent: new (await import('https-proxy-agent')).default(proxy) }
                : (() => { const agent = getProxyAgent(url); return agent ? { agent } : {}; })()
            ),
        };
...
export async function imagelabs(messages, model) {
    const imageLabs = new ImageLabs();
    try {
        const result = await imageLabs.request(model, messages, {
